...
class Attacker(EvaluationBasedAgent):
    ...
    def chooseAction(self, gameState):
        """
        Strategy:
        1. Give up last two foods.
        2. BFS eat enemy when ally not around.
        3. Greedy eat foods when safe.
        4. Greedy eat capsule when half nearestGhostDistance closer than enemy.
        5. Other situation: UCT Algorithm trade off score, escape, capsule.
        """
        ...
        if len(self.getFood(gameState).asList()) <= 2:
            uctAction = self.getUCTActions(gameState)
            # print "FINAL TWO FOOD TIME ", datetime.datetime.utcnow() - begin
            return uctAction

        """
        Strategy 2: pacman can BFS eat enemy capsule when:
        ...
        """
        Strategy 3: pacman can greedy eat foods when:
        ...
        if nearestGhost is None or (nearestGhost is not None and myGhostDist >= 6) or nearestGhost.scaredTimer > 5:
            if (nearestFood is not None and nearFoodsNum >= 5 and gameState.getAgentState(self.index).numCarrying <= 15) \
                ...
                eatAction = self.getEatAction(gameState, nearestFood)
                # print "EAT FOOD TIME ", datetime.datetime.utcnow() - begin
                return eatAction

        """
        Strategy 4: pacman can greedy eat capsule when:
        ...
        if nearestGhost is not None and not nearestGhost.isPacman and \
            ...
            eatAction = self.getEatAction(gameState, nearestCapsule)
